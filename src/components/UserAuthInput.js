...
const UserAuthInput = ({
  ...
  const [isEmilValid, setIsEmilValid] = useState(false);
      ...
      <div className="flex flex-col items-start justify-start gap-1">
        ...
        <div
          className={`flex items-center justify-center gap-3 w-full md:w-96 rounded-md px-4 py-1 bg-gray-200 ${
            !isEmilValid && placeHolder === 'Email' && value.length > 0 && 'border-2 border-red-500'
          }`}
        >
          ...
          {isPass && (
            <motion.div whileTap={{scale:.9}} onClick={()=> setShowPass(!showPass)} className="cursor-pointer">
